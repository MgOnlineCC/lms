...
$pdf->Cell(0,5,'SMA PASUNDAN 1 BANDUNG','0','1','C',false);
$pdf->SetFont('Arial','i',8);
$pdf->Cell(0,5,'Alamat : Jl. Balonggede No. 28 Kota Bandung, Indonesia','0','1','C',false);
$pdf->Cell(0,2,'Telp : 
022-4235729 - Email : smapasundansatubandung@gmail.com','0','1','C',false);
$pdf->Ln(3);
...
if(@$_GET['data'] == "pengajar") {
	...
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(50,5,'Laporan Daftar Kelas, Letak Ruang, Wali dan Ketua Masing-masing Kelas di SMA PASUNDAN 1 BANDUNG','0','1','L',false);
	$pdf->Ln(3);
	...
	while($data_siswa_mengikuti_tes = mysqli_fetch_array($sql_siswa_mengikuti_tes)) {
		...
		if(mysqli_num_rows($sql_jwb) > 0) {
			if(mysqli_num_rows($sql_essay) > 0) {
				...
				$total = "Essay belum dikoreksi";
			}
		} else {
			// echo "Ujian ini tidak ada soal essay";
			$nilai_essay = "tidak ada essay";
			$total = ". Nilai total : [".$data_pilgan['presentase'] ."]";
		}
